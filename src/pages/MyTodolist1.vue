<template>
  <div class="box">
    ...
    <div class="rightSection">
      <div class="add">
        <span
          ><i class="el-icon-folder-opened"></i>{{ currentItem.listName }}</span
        >
        <el-input
...
export default {
  ...
  methods: {
    ...
    deleteItem(data) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          console.log("当前的id是:" + data);
          let index = this.List.findIndex((v) => v.id === data);
          console.log("当前要删除的元素在List里是第" + index + "个");
    ...
    submitData() {
      ...
      console.log("当前要寻找的元素在List里是第" + index + "个");
    ...
    deleteTodoItem(data) {
      this.$confirm(
        ...
        .then(() => {
          ...
          console.log("当前的小todo的id是:" + data.id);
          let index = this.currentItem.todoItemList.findIndex(
            (v) => v.id === data.id
          );
          console.log("当前要删除的元素在todoItemList里是第" + index + "个");
          this.currentItem.todoItemList.splice(index, 1);
  ...
  created() {
    if (localStorage.getItem("todolist")) {
      ...
      console.log("localstorage里有数据,todolist拿取");
    }
  },
  computed: {
